<!DOCTYPE html>
<html lang="en">
    <head>
        <meta charset="UTF-8" />
        <meta name="viewport" content="width=device-width, initial-scale=1.0" />
        <script src="https://kit.fontawesome.com/a81368914c.js"></script>
        <link rel="stylesheet" href="CSS/sabtnam.css" />
        <link rel="stylesheet" href="CSS/font_imports.css" />
        <title>ثبت نام</title>
    </head>
    <body class="iranSans" style="background: url(./img/bgc2.jpg);">
        <img class="wave" src="img/wave.png" />
        <div class="container">
            <div class="img">
                <img src="./img/undraw_art_lover_yjfr.svg" />
            </div>
            <div class="login-content">
                <form action="index.html">
                    <img src="./img/undraw_profile_pic_ic5t.svg" />
                    <h2 class="title">ثبت نام</h2>
                    <div class="input-div one">
                        <div class="i">
                            <i class="fas fa-pen"></i>
                        </div>
                        <div class="div">
                            <h5></h5>
                            <input placeholder="نام" type="text" class="input" />
                        </div>
                    </div>
                    <div class="input-div pass">
                        <div class="i">
                            <i class="fas fa-pen"></i>
                        </div>
                        <div class="div">
                            <h5></h5>
                            <input placeholder="نام خانوادگی" type="password" class="input" />
                        </div>
                    </div>
                    <div class="input-div pass">
                        <div class="i">
                            <i class="fas fa-envelope"></i>
                        </div>
                        <div class="div">
                            <h5></h5>
                            <input placeholder="ایمیل" type="password" class="input" />
                        </div>
                    </div>
                    <div class="input-div pass">
                        <div class="i">
                            <i class="fas fa-lock"></i>
                        </div>
                        <div class="div">
                            <h5></h5>
                            <input placeholder="رمز عبور" type="password" class="input" />
                        </div>
                    </div>
                    <a class="qqqqq" href="singin.html">ورود</a>
                    <input type="submit" class="btn" value="ثبت نام" />
                </form>
            </div>
        </div>

        <script src="./js/sabtnam.js"></script>
    </body>
</html>
